  ...
  $.fn.tableExport = function (options) {
    ...
    if ( defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt' ) {
      ...
      $($rows).each(function () {
                               ...
                               for ( var styles in defaults.mso.styles ) {
                                 ...
                                 if ( defaults.mso.styles.hasOwnProperty(styles) ) {
                                   ...
                                   if ( thcss !== '' && thcss !== '0px none rgb(0, 0, 0)' && thcss !== 'rgba(0, 0, 0, 0)' ) {
                                     style["css"] = defaults.mso.styles[styles];
                                     style["value"] = thcss;
        ...
        data.push(cols);
        styleData.push(styleCols);
        rowIndex++;
      });

      //noinspection JSPotentiallyInvalidConstructorUsage
      var wb = new jx_Workbook(), ws = jx_createSheet(data, styleData);
      ...
      var wbout = XLSX.write(wb, {type: 'binary', bookType: defaults.mso.fileFormat, bookSST: true, cellStyles: true});
    ...
    function jx_createSheet (data, styleData) {
      var ws    = {};
      ...
      for ( var R = 0; R !== data.length; ++R ) {
        for ( var C = 0; C !== data[R].length; ++C ) {
          ...
          for(var i = 0;i < style.length; i++){
            ...
            if(tmp["css"] == "width"){
              if(R == 0){
                cols.push({"wpx" : Math.ceil(Number(tmp["value"].replace("px", "")) * 0.8)});
              }
            }else if(tmp["css"].indexOf("font") != -1){
              if(tmp["css"].indexOf("size") != -1){
                var tmp2 = {};

                tmp2["sz"] = Math.ceil(Number(tmp["value"].replace("px", "")));
                cell.s["font"] = tmp2;
              }else if(tmp["css"].indexOf("color") != -1){
                var tmp2 = {};

                tmp2["color"] = tmp["value"];
                cell.s["font"] = tmp2;
              }else if(tmp["css"].indexOf("weight") != -1){
                var tmp2 = {};

                tmp2["bold"] = Number(tmp["value"]) > 500 ? true : false;
                cell.s["font"] = tmp2;
              }
            }else if(tmp["css"].indexOf("background") != -1){
              if(tmp["css"].indexOf("color") != -1){
                var tmp2 = {};
                rgb
                tmp2["bgColor"] = {"rgb" : tmp["value"].replace("#", "")};
                cell.s["fill"] = tmp2;
              }
            }else if(tmp["css"].indexOf("text") != -1){
              if(tmp["css"].indexOf("align") != -1){
                var tmp2 = {};

                tmp2["horizontal"] = tmp["value"];
                cell.s["alignment"] = tmp2;
              }
            }else if(tmp["css"].indexOf("border") != -1){
              if(tmp["css"].indexOf("color") != -1){
                var tmp2 = {};
                var tmp3 = {};

                tmp3["color"] = tmp["value"];
